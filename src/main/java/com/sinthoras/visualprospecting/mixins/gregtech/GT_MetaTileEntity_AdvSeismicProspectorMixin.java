            ...
            if (ready && mMaxProgresstime == 0) {
                ...
                for (int i = -chunkRadius; i < chunkRadius + 1; i++) {
                    for (int j = -chunkRadius; j < chunkRadius + 1; j++) {
                        ...
                        final GT_OreVeinStats.Stats stats = GT_OreVeinStats.getOreVeinStatsInChunk(aBaseMetaTileEntity.getWorld(), chunkCoordX, chunkCoordZ);

                        VeinType veinType;
                        if (stats != null) {
                            veinType = VeinTypeCaching.getVeinType(stats.oreMix().unlocalizedName());
                        } else {
...
//            Item compoundExp = GameRegistry.findItem("htx","item.explosivecompound");
            Item compoundExp = Items.gunpowder;
